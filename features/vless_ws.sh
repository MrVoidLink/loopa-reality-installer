create_vless_ws_inbound() {
  clear
  echo "Create new VLESS WebSocket inbound (security=none)"
  ...
  read -rp "Enter public host/IP for client link [${DETECTED_SERVER_ADDR:-required}]: " SERVER_ADDR
  SERVER_ADDR=${SERVER_ADDR:-$DETECTED_SERVER_ADDR}
  [ -z "${SERVER_ADDR:-}" ] && err "Public host/IP is required."
  SERVER_ADDR=$(clean_input "$SERVER_ADDR")
  validate_host_or_ip "$SERVER_ADDR"

  read -rp "Enter WebSocket host header [vip.proyaar.ir]: " WS_HOST
  WS_HOST=${WS_HOST:-vip.proyaar.ir}
  ...
  LINK="vless://${UUID}@${SERVER_ADDR}:${PORT}?encryption=none&security=none&type=ws&host=${LINK_HOST}&path=${LINK_PATH}#${SAFE_LINK_NAME}"
  INFO_FILE="$DATA_DIR/loopa-vless-ws-${PORT}.txt"
  CLIENT_JSON_FILE="$DATA_DIR/loopa-vless-ws-client-${PORT}.json"
...
EOF
  ...
  echo ""
  echo "Note: if the client link address is different from this server's real public IP/domain,"
  echo "you must place a reverse proxy/CDN/fronting layer in front of this Xray listener."
